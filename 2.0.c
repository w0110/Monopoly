...
int buy(int player, int ownInt[25], char ownChar[25], int land_value[25], int level[25], int pay[25], int move, int money)
{
	int chose;
	//int pay_list[25] = {0, 200, 300, 500, 600, 700, 800, 900, 1000, 1200, 1400, 1600, 1800, 2000, 2300, 2500, 3000};
	char name[25][100] = {" ", "水星", "金星", " ", "火星", "木星", "土星", " ", "天王星", " ", "海王星", "駭客星", " ", "月球", "植物星", " ", "方塊星", "美食星", "逢甲星", " ", "旺旺星", " ", "神秘星", "地球"}
	char player[5][100] = {" ", "player1", "player2", "player3", "plaver4"};
	if(ownerInt[move] == 0)	//此地尚未有地主時 
	{
		printf("Do you want to buy this planet ?\n Yes : please enter 1\n No : please enter 0\n");
		scanf("%d", &chose);
		if(enter == 0)	//
			return 0;
		if(enter == 1 && money > land_value[move])
		{
			printf("cost : %d", land_value[move]);
			money = money - land_value[move];	//現有的錢 = 原有的錢 - 地價 
			level[move]++;	//等級從 0 變 1 
			pay[move] = land_value[move] / 2;	//過路費 = 地價 / 2 
			printf("you still have %d dollars", money);
			printf("datas about %s : level %d , toll : %d", name[move], level, pay[move]);
		}
	}
	if(ownerInt[move] == player)	//此地為自己的地時 
	{
		printf("current level : %d \n Do you want to upgrade ? \n Yes : please enter 1\n No : please enter 0\n", level[move]);	//詢問是否升等 
		scanf("%d", %chose);
		if(enter == 0)	//不升等 
			return 0;
		if(enter == 1 && money > land_value[move])	//要升等 且 現有的錢 > 地價 
		{
			printf("you have to pay %d dollars to upgrade %s ", land_value[move] * 2, name[move]);
			money = money - land_value[move] * 2;
			printf("Now you still have %d dollars", money);
		}
	}
	if(ownerInt[move] != player)	//此地為別人的地時 
	{
		printf("You have to pay %d to %s", pay[move], player[ownerInt]);
